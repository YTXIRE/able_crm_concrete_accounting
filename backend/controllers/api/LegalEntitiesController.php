...
class LegalEntitiesController extends Controller
{
    ...
    /**
     * @OA\Get(
     *     path="/api/legal-entities/get-all",
     *     summary="Получение всех юридических лиц",
     *     operationId="get-all",
     *     tags={"legal-entities"},
     *     @OA\Parameter(
     *         name="token",
     ...
     *                                  "id": 1,
     *                                  "name": "Организация 1",
     *                                  "legal_entities_type": "ОАО",
     *                                  "legal_entities_type_id": 2
     *                              }
     *                          },
     *                          "count": 1
     *                      }
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Юридические лица могут быть либо активные - 0, либо архивные 1",
     *                 summary="",
     *                 value={
     *                     "code": 400,
     *                     "status": "Bad Request",
    ...
    function actionGetAll($token = '', $user_id = 0, $limit = 0, $offset = 0, $archive = 0): Response
    {
        try {
            ...
            if ($archive !== 0 && $archive !== 1) {
                return General::generalMethod($request, 400, [$archive], $this, Constants::$LEGAL_ENTITY_ARCHIVE_OR_ACTIVE);
            }
            ...
            $legal_entities_tmp = LegalEntities::getAll($limit, $offset, $archive);
            $legal_entities = [
                'legal_entities' => [],
                'count' => 0
            ];
            foreach ($legal_entities_tmp as $legal_entity) {
                $legal_entities['legal_entities'][] = [
                    'id' => $legal_entity['id'],
                    'name' => $legal_entity['name'],
                    'legal_entities_type' => $legal_entity->type['name'],
                    'legal_entities_type_id' => $legal_entity->type['id'],
                    'full_name' => $legal_entity->type['name'] . " " . $legal_entity['name']
                ];
            }
            $legal_entities['count'] = (int)LegalEntities::getAllCount($archive);
            return General::success($legal_entities, $request, $this);
        } catch (Exception $e) {
    ...
    /**
     * @OA\Post(
     *     path="/api/legal-entities/create",
     *     summary="Создание нового юридического лица",
     *     operationId="create",
     *     tags={"legal-entities"},
     *     @OA\RequestBody(
     ...
     *                 ),
     *                 @OA\Property(
     *                     property="legal_entities_type_id",
     *                     description="ID типа организации",
     *                     type="integer"
     *                 ),
     *                 @OA\Property(
     *                     property="user_id",
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Пожалуйста, укажите наименование юридического лица",
     *                 summary="",
     *                 value={
     *                     "code": 400,
     *                     "status": "Bad Request",
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Максимальная длина названия юридического лица может быть 20 символов",
     *                 summary="",
     *                 value={
     *                     "code": 400,
     *                     "status": "Bad Request",
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Тип организации с указанным идентификатором не найден",
     *                 summary="",
     *                 value={
     *                     "code": 404,
     *                     "status": "Not Found",
    ...
    function actionCreate(): Response
    {
        try {
            ...
            $data = [
                ...
                'legal_entities_type_id' => array_key_exists('legal_entities_type_id', $data) ? (int)($data['legal_entities_type_id']) : 0,
            ...
            if (!LegalEntitiesTypes::checkIsNotExitsId($data['legal_entities_type_id'])) {
                return General::generalMethod($request, 404, $data, $this, Constants::$TYPE_ORGANIZATION_WITH_THIS_ID_WAS_NOT_FOUND);
            }
            if (!Users::checkUserWithTokenAndID(['id' => $data['user_id'], 'token' => $data['token']])) {
                return General::generalMethod($request, 404, [], $this, Constants::$USER_WITH_TOKEN_AND_ID_NOT_FOUND);
            }
            $result = LegalEntities::saveLegalEntities($data);
            if ($result === false) {
                throw new Exception('Ошибка в базе данных');
            }
            return General::generalMethod($request, 201, [], $this, Constants::$NEW_LEGAL_ENTITY_HAS_BEEN_SUCCESSFULLY_CREATED);
        } catch (Exception $e) {
            return General::generalMethod($request, 500, $e, $this, Constants::$INTERNAL_SERVER_ERROR);
        }
    }
    ...
    /**
     * @OA\Put(
     *     path="/api/legal-entities/update",
     *     summary="Обновление юридического лица",
     *     operationId="update",
     *     tags={"legal-entities"},
     *     @OA\RequestBody(
     *         description="Формат входных данных",
     ...
     *              @OA\Examples(
     *                 example="",
     *                 summary="Обновлять можно только активные юридические лица",
     *                 value={
     *                     "code": 400,
     *                     "status": "Bad Request",
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Организация с указанным идентификатором не найдена",
     *                 summary="",
     *                 value={
     *                     "code": 404,
     *                     "status": "Not Found",
    ...
    function actionUpdate(): Response
    {
        try {
            ...
            if (!LegalEntities::checkIsNotExitsId($data['id'])) {
                return General::generalMethod($request, 404, $data, $this, Constants::$ORGANIZATION_WITH_THIS_ID_WAS_NOT_FOUND);
            }
            if (LegalEntities::checkIsArchive($data['id'])) {
                return General::generalMethod($request, 400, $data, $this, Constants::$LEGAL_ENTITY_WAS_UPDATED_ERROR);
            }
            if (!Users::checkUserWithTokenAndID(['id' => $data['user_id'], 'token' => $data['token']])) {
                return General::generalMethod($request, 404, [], $this, Constants::$USER_WITH_TOKEN_AND_ID_NOT_FOUND);
            }
            $result = LegalEntities::updateLegalEntities($data);
            if ($result === false) {
                throw new Exception('Ошибка в базе данных');
            }
    ...
    /**
     * @OA\Put(
     *     path="/api/legal-entities/restore",
     *     summary="Восстановление юридического лица",
     *     operationId="restore",
     *     tags={"legal-entities"},
     *     @OA\RequestBody(
     *         description="Формат входных данных",
     ...
     *                     "code": 200,
     *                     "status": "OK",
     *                     "message": "Юридическое лицо успешно восстановленоо"
     *                  }
     *              )
     *          )
     *     ),
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Юридическое лицо с указанным идентификатором уже было восстановлено",
     *                 summary="",
     *                 value={
     *                     "code": 400,
     *                     "status": "Bad Request",
    ...
    function actionRestore(): Response
    {
        try {
            ...
            if (!LegalEntities::checkIsNotExitsId($id)) {
                return General::generalMethod($request, 404, $data, $this, Constants::$ORGANIZATION_WITH_THIS_ID_WAS_NOT_FOUND);
            }
            if (!Users::checkUserWithTokenAndID(['id' => $user_id, 'token' => $token])) {
                return General::generalMethod($request, 404, [], $this, Constants::$USER_WITH_TOKEN_AND_ID_NOT_FOUND);
            }
            if (!LegalEntities::checkIsArchive($id)) {
                return General::generalMethod($request, 400, $data, $this, Constants::$LEGAL_ENTITY_WAS_RESTORED);
            }
            if (!LegalEntities::setIsArchive($id, 0)) {
                throw new Exception('Ошибка в базе данных');
            }
            return General::generalMethod($request, 200, $data, $this, Constants::$LEGAL_ENTITY_SUCCESSFULLY_RESTORED);
        } catch (Exception $e) {
            return General::generalMethod($request, 500, $e, $this, Constants::$INTERNAL_SERVER_ERROR);
        }
    }
    ...
    /**
     * @OA\Delete(
     *     path="/api/legal-entities/delete",
     *     summary="Удаление юридического лица",
     *     operationId="delete",
     *     tags={"legal-entities"},
     *     @OA\RequestBody(
     *         description="Формат входных данных",
     ...
     *              ),
     *              @OA\Examples(
     *                 example="Юридическое лицо с указанным идентификатором уже было удалено",
     *                 summary="",
     *                 value={
     *                     "code": 400,
     *                     "status": "Bad Request",
    ...
    function actionDelete(): Response
    {
        try {
            ...
            if (LegalEntities::checkIsArchive($id)) {
                return General::generalMethod($request, 400, $data, $this, Constants::$LEGAL_ENTITY_WAS_REMOVED);
            }
            if (!LegalEntities::setIsArchive($id, 1)) {
                throw new Exception('Ошибка в базе данных');
            }
            return General::generalMethod($request, 200, $data, $this, Constants::$LEGAL_ENTITY_SUCCESSFULLY_REMOVED);
        } catch (Exception $e) {
            return General::generalMethod($request, 500, $e, $this, Constants::$INTERNAL_SERVER_ERROR);
        }
    }
