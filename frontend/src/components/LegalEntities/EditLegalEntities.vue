<template>
    <div>
        ...
        <el-dialog v-model="dialogVisible" title="Редактирование юридического лица">
            <el-form ref="updateLegalEntities" :model="fields" label-width="130px" @submit.prevent.stop>
                <el-form-item
                    :disabled="loading"
...
export default {
    ...
    methods: {
        ...mapActions(["getAllLegalEntitiesTypes", "updateLegalEntities"]),
        submitForm: function () {
            this.$refs["updateLegalEntities"].validate(async (valid) => {
                if (valid) {
                    this.loading = true;
                    const result = await this.updateLegalEntities({
                        id: this.data.id,
    ...
    async mounted() {
        ...
        this.fields.legal_entities_type_id = this.data.legal_entities_type_id;
        this.is_demo = +localStorage.getItem("is_demo") === 0;
    },
    watch: {
        data(e) {
            this.fields.name = e.name;
            this.fields.legal_entities_type_id = e.legal_entities_type_id;
        },
    },
};
</script>
