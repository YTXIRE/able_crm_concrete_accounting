    ...
    <el-dialog v-model="dialogVisible" fullscreen title="Настройка представления">
                    ...
                    >
                        ...
                        <el-card class="card" shadow="hover">
                            ...
                            <div class="card_block">
                                ...
                                <br />
                                <el-select
                                    v-if="check_field_type(block.field) === 'selector'"
                                    v-model="block.value"
        ...
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="updateFilterMethod"> Сохранить </el-button>
            </span>
        </template>
    </el-dialog>
</template>
...
export default {
        ...
        updateFilterMethod: async function() {
            ...
            if (errors.length === 0) {
                if (this.filters_block.length !== 0) {
                    ...
                    const response = await this.updateFilter({
    ...
    mounted() {
        this.setValue();
        this.filter_name = this.data.name;
        const fb = JSON.parse(JSON.stringify(this.data.filters));
        for (let item in fb) {
            if (String(fb[item]["field"]).startsWith("date_")) {
                fb[item]["value"] = String(new Date(fb[item]["value"] * 1000));
            }
        }
        this.filters_block = fb;
        this.filters_data = {
            ...this.filters_data,
    ...
    watch: {
        ...
        data(e) {
            ...
            const fb = JSON.parse(JSON.stringify(e.filters));
            for (let item in fb) {
                if (String(fb[item]["field"]).startsWith("date_")) {
                    fb[item]["value"] = String(new Date(fb[item]["value"] * 1000));
                }
